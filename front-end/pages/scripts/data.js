...
const DataStore = (() => {
    ...
    async function getCustomerByPhoneAsync(phone) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/customers?companyId=${encodeURIComponent(companyId)}`, {
                method: 'GET',
                headers: {
    ...
    async function applyPromo(code, subtotal) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:4000/api/orders/promotions/validate', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-role': role
