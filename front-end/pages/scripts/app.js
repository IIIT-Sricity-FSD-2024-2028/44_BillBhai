...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function resolveBackendCustomer(dataPayload, companyId, userRole) {
        ...
        if (normalizedPhone) {
            try {
                const scopedResponse = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/customers?companyId=${encodeURIComponent(companyId)}`, {
                    method: 'GET',
                    headers: {
                    ...
                    if (existing && existing.id) {
                        ...
                        if (needsUpdate) {
                            try {
                                ...
                                const updateResponse = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/customers/${encodeURIComponent(String(existing.id))}`, {
                                    method: 'PUT',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'x-role': userRole
        ...
        try {
            ...
            const createResponse = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/customers`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-role': userRole
    ...
    async function submitOrderToBackend(dataPayload) {
        try {
            ...
            const response = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/orders`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-role': userRole
            ...
            if (orderType === 'delivery') {
                ...
                try {
                    const deliveryResponse = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/deliveries`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'x-role': userRole
            ...
            try {
                const updateResponse = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/orders/${encodeURIComponent(String(backendOrder.id))}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'x-role': userRole
            ...
            try {
                const listResp = await fetch(`${window.BILLBHAI_CONFIG.API_BASE_URL}/orders?companyId=${encodeURIComponent(companyId)}`, {
                    method: 'GET',
