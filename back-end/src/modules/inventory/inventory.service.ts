...
export class InventoryService {
  ...
  findByProduct(productId: string) {
    ...
    if (!item) throw new NotFoundException(`No inventory record for product ${productId}`);
    return item;
  }

  findLowStock() {
    return this.inventory.filter(i => i.status === 'Low Stock' || i.status === 'Critical' || i.status === 'Out of Stock');
  }

  update(id: string, dto: UpdateInventoryDto) {
    const idx = this.inventory.findIndex(i => i.id === id);
    if (idx === -1) throw new NotFoundException(`Inventory item ${id} not found`);
  ...
  adjustStock(dto: AdjustStockDto) {
    ...
    if (idx === -1) throw new NotFoundException(`No inventory for product ${dto.productId}`);
    ...
    this.inventory[idx].status = this.computeStatus(newStock, this.inventory[idx].reorderLevel);
